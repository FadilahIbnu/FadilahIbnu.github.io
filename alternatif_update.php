...
while ($r = mysqli_fetch_array($q_kriteria)) {
    ...
    while ($r2 = mysqli_fetch_array($q2)) {
        $combo_subkriteria .= '<option value="' . $r2['id_subkriteria'] . '">' . $r2['nama_subkriteria'] . '</option>';
    }
...
if (isset($_POST['save'])) {
    ...
    if ($action == 'add') {
        if (mysqli_num_rows(mysqli_query($con, "select * from alternatif where nama_alternatif='" . $nama_alternatif . "'")) > 0) {
            ...
            while ($r2 = mysqli_fetch_array($q2)) {
                ...
                $q = "insert into alternatif_kriteria(id_alternatif,id_kriteria,id_subkriteria) values('" . $id_alternatif . "','" . $r2['id_kriteria'] . "','" . $id_subkriteria . "')";
                mysqli_query($con, $q);
            }
    ...
    if ($action == 'edit') {
        ...
        if (mysqli_num_rows(mysqli_query($con, "select * from alternatif where nama_alternatif='" . $nama_alternatif . "' and nama_alternatif<>'" . $nama_alternatif_tmp . "'")) > 0) {
            ...
            while ($r2 = mysqli_fetch_array($q2)) {
                $id_subkriteria = $kriteria[$r2['id_kriteria']];
                if (mysqli_num_rows(mysqli_query($con, "select * from alternatif_kriteria where id_alternatif='" . $id . "' AND id_kriteria=" . $r2['id_kriteria'])) > 0) {
                    $q = "update alternatif_kriteria set id_subkriteria='" . $id_subkriteria . "' where id_alternatif='" . $id . "' AND id_kriteria=" . $r2['id_kriteria'];
                    mysqli_query($con, $q);
                } else {
                    $q = "insert into alternatif_kriteria(id_alternatif,id_kriteria,id_subkriteria) values('" . $id . "','" . $r2['id_kriteria'] . "','" . $id_subkriteria . "')";
                    mysqli_query($con, $q);
                }
            }
            exit("<script>location.href='" . $link_data . "';</script>");
    ...
    if ($action == 'edit') {
        ...
        $q2 = mysqli_query($con, $q2);
        while ($r2 = mysqli_fetch_array($q2)) {
            $rk = mysqli_fetch_array(mysqli_query($con, "select * from alternatif_kriteria where id_kriteria=" . $r2['id_kriteria'] . " AND id_alternatif=" . $id));
        ...
        while ($r = mysqli_fetch_array($q_kriteria)) {
            ...
            $q2 = mysqli_query($con, $q2);
            while ($r2 = mysqli_fetch_array($q2)) {
                if ($r2['id_subkriteria'] == $kriteria[$id_kriteria]) {
                    $selected = "selected";
                } else {
                    $selected = "";
                }
