...
$bobot = '';
$combo_tipe = '';
$combo_tipe .= '<select class="form-control" name="tipe" id="tipe" required><option value="">Pilih...</option>';
$arr = array("cost", "benefit");
foreach ($arr as $arrdata) {
    $combo_tipe .= '<option value="' . $arrdata . '">' . $arrdata . '</option>';
}
$combo_tipe .= '</select>';
...
if (isset($_POST['save'])) {
    ...
    if ($action == 'edit') {
        ...
        foreach ($arr as $arrdata) {
            ...
            $combo_tipe .= '<option value="' . $arrdata . '" ' . $selected . '>' . $arrdata . '</option>';
        }
        $combo_tipe .= '</select>';
    }
    if ($action == 'delete') {
    ...
    <form class="form-horizontal" action="<?php echo $link_update; ?>" method="post">
        ...
        <div class="box-body">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-4">
                    <input name="bobot" id="bobot" required type="number" step="0.1" class="form-control" value="<?php echo $bobot; ?>">
                </div>
            </div>
            <div class="form-group">
