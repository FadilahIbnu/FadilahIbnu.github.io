...
if (isset($_POST['save'])) {
    ...
    if (empty($error)) {
        if (mysqli_num_rows(mysqli_query($con, "select * from pengguna where id_pengguna='" . $id_pengguna . "' and password='" . md5($password) . "'")) > 0) {
            mysqli_query($con, "update pengguna set password='" . md5($password_baru) . "' where id_pengguna='" . $id_pengguna . "'");
