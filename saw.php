...
while ($ralternatif = mysqli_fetch_array($sql2)) {
    ...
    while ($rkriteria = mysqli_fetch_array($query2)) {
        $rk = mysqli_fetch_array(mysqli_query($con, "select id_subkriteria from alternatif_kriteria where id_alternatif='" . $ralternatif['id_alternatif'] . "' AND id_kriteria='" . $rkriteria['id_kriteria'] . "'"));
...
while ($r2 = mysqli_fetch_array($sql2)) {
    ...
    if (mysqli_num_rows(mysqli_query($con, "select * from hasil where id_alternatif='" . $id_alternatif . "'")) > 0) {
        $q = "update hasil set nilai='" . $nilai_v . "' where id_alternatif='" . $id_alternatif . "'";
        mysqli_query($con, $q);
    } else {
        $q = "insert into hasil(id_alternatif,nilai) values ('" . $id_alternatif . "','" . $nilai_v . "')";
        mysqli_query($con, $q);
    }
